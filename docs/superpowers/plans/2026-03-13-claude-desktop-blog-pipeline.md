...
**Architecture:** Claude Desktop이 매일 09:00 KST에 Supabase MCP로 content_calendar의 당일 pending 항목을 조회하고, 완성도 높은 블로그 초안을 생성하여 posts 테이블에 insert. 기존 Gemini cron은 시간을 분리하여 병행 운영.
...
**배경:** 현재 Gemini cron이 `0 0 * * *` (UTC) = 09:00 KST로 설정되어 있어 Claude Desktop 스케줄(09:00 KST)과 충돌. 12:00 KST (03:00 UTC)로 이동하여 Claude가 먼저 처리할 시간을 확보한다.
...
**배경:** Claude Desktop의 스케줄 기능으로 매일 09:00 KST에 자동 실행될 프롬프트를 등록한다.
