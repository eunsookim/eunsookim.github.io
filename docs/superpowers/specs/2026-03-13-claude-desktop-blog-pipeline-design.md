...
[Claude Desktop Schedule — 매일 09:00 KST]
...
- **인증:** `SUPABASE_SERVICE_ROLE_KEY` (기존 .env에서 사용 중인 키)
- **접근 테이블:** `content_calendar` (SELECT, UPDATE) + `posts` (INSERT) + `categories` (SELECT)
...
- service_role_key는 RLS 우회 — 프롬프트에서 접근 테이블을 명시적으로 제한
- 로컬 머신에서만 실행, 키 노출 위험은 기존 .env 관리 수준과 동일
...
- **slug:** 제목에서 영문/숫자 기반 kebab-case 자동 생성
...
- Gemini cron은 Vercel Cron으로 별도 시간에 실행 (충돌 방지를 위해 실행 시간 분리 권장)
...
1. **`src/app/admin/(dashboard)/posts/page.tsx`** — 글 목록에 `generated_by` 뱃지 추가 (AI / Human 시각적 구분)
2. **`src/app/admin/(dashboard)/calendar/page.tsx`** — CalendarItemCard의 generated 항목에 해당 글 에디터로 이동하는 링크 버튼 추가
...
| **설정** | Claude Desktop 스케줄 등록 | 매일 09:00 KST 프롬프트 실행 |
| **코드** | 글 목록 `generated_by` 뱃지 | `/admin/posts` 페이지에 AI/Human 뱃지 추가 |
